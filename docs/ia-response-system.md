...
- **Streaming:** "pensando" ~2–3s antes (menos p/ dose/crítico), revelação por unidades (~48ms). **Parar** congela e marca "Resposta interrompida" → **Continuar** retoma do ponto exato. **Parar no "pensando"** (antes do 1º token) deixa "Geração cancelada → **Tentar de novo**" (não some a pergunta órfã). Guard síncrono evita **envio duplicado** (double-tap). Progresso efêmero (nunca persiste cru). Respeita `prefers-reduced-motion` (a fazer: pular streaming).
