...
- **Estado vazio:** **marca-herói** centralizado (ícone `ia` + "CalcMed IA" / "Assistente clínico"), saudação por horário, título e texto. **Gradiente "aurora"** (três radiais suaves em tints do `--ds-interativo-primario`) **só no vazio** (`.conversation[data-empty]`) — some sozinho ao iniciar a conversa.
