...
- **Fundo branco** (`.screen` = `--ds-fundo-cartao` no light; no dark mantém `--ds-fundo-padrao` via `:global(.modo-escuro)`). Cards e input "sobem" por **sombra** (`--sombra-cartao`), não por contraste de fundo.
- **Estado vazio:** **marca-herói** centralizado (ícone `ia` + "CalcMed IA" / "Assistente clínico"), saudação por horário, título e texto. **Gradiente "aurora"** (radial central grande e macio que dissolve atrás da marca + 2 acentos nos cantos; cor via `color-mix` sobre `--ds-interativo-primario`, ~12/8/5% — mais presença que a escala fixa de tints, e adapta ao dark) **só no vazio** (`.conversation[data-empty]`) — some sozinho ao iniciar a conversa.
- **Sugestões iniciais:** seção "Comece por aqui" + subtítulo, com **grid de 2 linhas rolando na horizontal** — **12 cards** (ícone do DS em tile + label de até 2 linhas + chevron). Colunas largas (~56%) → a 2ª coluna fica **cortada na borda**, sinalizando rolagem. Só no estado vazio, acima do composer.
...
- **Composer** multilinha (`textarea` auto-resize 1→~5 linhas; Enter envia, Shift+Enter quebra). Input **maior, com sombra** (flutua no branco) e **aviso de evidências fixo embaixo** ("Respostas baseadas em evidências. Valide com seu julgamento.") — persistente em qualquer estado, não só no vazio.
